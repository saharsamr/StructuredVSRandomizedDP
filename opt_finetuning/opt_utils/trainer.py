...
from svd_exp_utils.galore_adamw_modified import AdamW as GaLoreAdamWSVDExp  # For SVD exp
...
class OurTrainer(Trainer):
    ...
    def create_optimizer(self):
        """
        Based on Transformers' default one, add dp setup
        """
        ...
        if self.optimizer is None:
            ...
            if self.args.dpgrape or self.args.svd_exp_galore or self.args.svd_exp_naive_dpgalore:
                galore_params = []
                target_modules = ["attn", "attention", "dense", "mlp", "fc", "lm_head"]
                skip_modules = ["lm_head"]
                for module_name, module in self.model.named_modules():
                    #print(module_name, isinstance(module, torch.nn.Linear))
                    #print(module_name, isinstance(module, torch.nn.Linear), not any(target_key in module_name for target_key in target_modules), any(key in module_name for key in skip_modules))
                ...
                if self.args.svd_exp_galore or self.args.svd_exp_naive_dpgalore:
                    self.optimizer = GaLoreAdamWSVDExp(param_groups, 
    ...
    def _inner_training_loop(
        ...
        if self.args.dpzero or self.args.dpgrape or self.args.dpadam:
            ...
            logger.info(f"  Noise multiplier: {multiplier}")
            self.dpzero_gaussian_std = 2 * multiplier * self.args.dp_clip_threshold / total_train_batch_size

        # Freeze positional encodings
        target_params = ["model.decoder.embed_positions.weight"]
        for name, param in self.model.named_parameters():
            if name in target_params:
                param.requires_grad = False

        # Privacy setup for svd exp
        if self.args.svd_exp_naive_dpgalore:
            model.train()  # Required for opacus
            multiplier = self.args.svd_exp_sigma
            privacy_engine = PrivacyEngineModified()
            if args.svd_exp_no_clip:   # For experiments with noise but no clipping
                model, self.optimizer, train_dataloader = privacy_engine.make_private(
                    module=model,
        ...
        # Privacy setup for dpadam and dpgrape
        elif self.args.dpgrape:
            privacy_engine = PrivacyEngineModified()
            model.train()  # Required for opacus
            ...
            self.optimizer.update_projectors('gaussian')
            model.update_projectors(self.optimizer)
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if args.trainer == "zo":
                    ...
                    """if (
                        ...
                        with model.no_sync():
                            #print("BEFORE training step", torch.cuda.max_memory_allocated(), torch.cuda.max_memory_reserved())
                            tr_loss_step = self.training_step(model, inputs)
                    else:
                        #print("BEFORE training step", torch.cuda.max_memory_allocated(), torch.cuda.max_memory_reserved())
                        tr_loss_step = self.training_step(model, inputs)
                        #print("AFTER training step", torch.cuda.max_memory_allocated(), torch.cuda.max_memory_reserved())"""
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if args.trainer == "zo":
                        ...
                        # Gradient clipping
                        if args.max_grad_norm is not None and args.max_grad_norm > 0 and not self.args.dpgrape and not self.args.dpadam and not self.args.svd_exp_galore and not self.args.svd_exp_naive_dpgalore:
                            # deepspeed does its own clipping, dpgrape/dpadam do their own clipping
                        ...
                        # For svd exp, stop after first batch is done and save singular values
                        if self.args.svd_exp_galore or self.args.svd_exp_naive_dpgalore:
                            if self.args.svd_exp_naive_dpgalore:
                                galore_optimizer = self.optimizer.original_optimizer
                            else:
                                galore_optimizer = self.optimizer
                            save_items = {"layer_singular_values":[]}
                            for group in galore_optimizer.param_groups:
                                for p in group["params"]:
                                    if p.grad is None:
                                        continue
                                    state = galore_optimizer.state[p]
                                    if "rank" in group and "projector" in state:
                                        save_items["layer_singular_values"].append(state["projector"].singular_values.cpu())
                            
                            total_batch_size = self.args.per_device_train_batch_size * self.args.gradient_accumulation_steps
                            if self.args.svd_exp_naive_dpgalore and not self.args.svd_exp_no_clip:
                                file = "naive-dpgalore-svals-bs" + str(total_batch_size) + "-subspace-r" + str(self.args.subspace_r) + "-clipTrue-clipC" + str(self.args.dp_clip_threshold) + "-sigma" + str(self.args.svd_exp_sigma) + ".pt"
                            elif self.args.svd_exp_naive_dpgalore and self.args.svd_exp_no_clip:
                                file = "naive-dpgalore-svals-bs" + str(total_batch_size) + "-subspace-r" + str(self.args.subspace_r) + "-clipFalse-sigma" + str(self.args.svd_exp_sigma) + ".pt"
                            elif self.args.svd_exp_galore:
                                file = "galore-svals-bs" + str(total_batch_size) + "-subspace-r" + str(self.args.subspace_r) + ".pt"

                            save_path = os.path.join(self.args.svd_exp_save_folder_name, file)
                            torch.save(save_items, save_path)
                            return TrainOutput(self.state.global_step, None, None)
                    ...
                    if self.args.dpgrape or self.args.dpadam or self.args.svd_exp_naive_dpgalore:
                ...
                # Log dev loss
                if not self.args.no_eval and self.state.global_step % self.args.eval_steps == 0 and self.state.global_step != 0:
    ...
    def dpzero_step(self, model, inputs):
        ...
        self.projected_grad = self.dpzero_clip(loss_diff, self.args.dp_clip_threshold).mean()
