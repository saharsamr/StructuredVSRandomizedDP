...
from roberta_utils.modeling_roberta import RobertaConfig
from transformers import GlueDataTrainingArguments as DataTrainingArguments
from transformers import HfArgumentParser, TrainingArguments, set_seed

from roberta_utils.linearhead_trainer import LinearHeadTrainer
from roberta_utils.dataset import FewShotDataset, OurInputFeatures
...
class DynamicDataTrainingArguments(DataTrainingArguments):
    ...
    demo_filter_rate: float = field(
        default=0.5,
        metadata={"help": "Only use top-x percentage similar instances in demonstrations"}
    )
...
class DynamicTrainingArguments(TrainingArguments):
    ...
    gradient_accumulation_steps: int = field(
        default=1,
        metadata={"help": "number of gradient accumulation steps to do on each device"}
    )
    ...
    dp_clip_threshold: float = field(
        default=1.0,
        metadata={"help": "DPZero clip threshold"}
    )
    dp_clip_strategy: str = field(
        default='flat',
        metadata={"help": "how to clip sample gradients, options are flat or per_layer"}
    )
    ...
    dpadam: bool = field(
        default=False,
        metadata={
            'help': 'whether to use dpadam in training'
        }
    )
    ...
    # DP-Grape Projection Args
    dpgrape: bool = field(
        default=False,
        metadata={
            "help": 'whether to use DP-Grape in training'}
    )
    subspace_r : int = field(
        default=64,
        metadata={
            "help": "rank of subspace to use"}
    )
    subspace_T: int = field(
        default=250,
        metadata={
            "help": "how often to switch subspaces, in number of batches?"
        }
    )
...
def main():
    ...
    logging.basicConfig(
        ...
        datefmt="%m/%d/%Y %H:%M:%S",
        level=logging.INFO if "LOCAL_RANK" in os.environ else logging.WARN,
    )
